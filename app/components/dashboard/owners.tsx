        ...
        <div className="fixed absolute inset-0 bg-black/50 z-50 flex items-center justify-center">
          ...
          >
            <div className="flex py-2 px-4 justify-between border-b border-gray-300 sticky top-0 bg-gray-100 z-10">
              <p className="font-bold text-lg">Owners</p>
              <button
                ...
                <tbody>
                  {users.map((owner, index) => (
                    <tr key={index} className="hover:bg-gray-200">
                      <td className="px-1 py-2">{`${owner.firstName} ${owner.lastName}`}</td>
                      <td className="px-1 py-2">{owner.email}</td>
                      <td className="px-1 py-2">{owner.phone}</td>
                      <td>
                        <FaTrash
